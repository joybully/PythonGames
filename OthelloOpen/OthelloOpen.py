...
class GameLayer(cocos.layer.Layer):
    ...
    def update(self, dt):
        ...
        if self.turn == GameLayer.COMPUTER and self.count > 100:
                copyWeight = np.copy(self.weight)
                self.updateWeightA(self.table, GameLayer.COMPUTER, self.weight)
                self.computer()
                self.weight = copyWeight

    def updateWeightA(self, board, turn, weight):
    #count coin which can be converted
        for yy in range(0,8):
            for xx in range(0,8):
                rtnlist1 = self.isPossible(xx, yy, turn, board)
                myPoint = len(rtnlist1)
                rtnlist2 = self.isPossible(xx, yy, (-1)*turn, board)
                otherPoint = len(rtnlist2)
                if(myPoint+otherPoint == 0):
                    myScore = 0
                else:
                    myScore = 100*(myPoint-otherPoint)/(myPoint+otherPoint)
                weight[yy][xx] += myScore

    # def updateWeightB(self, board, turn, weight):
    # #count coin which can be on
    #     moves = []
    #     for y in range(0,self.row):
    #         for x in range(0,self.column):
    #             if board[y][x] != 0:continue

    #             myRevList = self.isPossible(x,y,turn,board)
    #             otherRevList = self.isPossible(x,y,(-1)*turn,board)
    #             if len(myRevList)+len(otherRevList)>0:
